...
# FIXME: these are almost certainly incorrect
...
class Py_intptr_t_converter(CConverter):
    ...
    def converter_init(self, *, accept={int}):
        if accept == {int}:
            ...
            fail("Py_intptr_t_converter: illegal 'accept' argument " + repr(accept))
...
class Py_uintptr_t_converter(CConverter):
    type = 'Py_uintptr_t'
    c_ignored_default = "0"
    ...
    def converter_init(self, *, accept={int}):
        if accept == {int}:
            ...
            fail("Py_uintptr_t_converter: illegal 'accept' argument " + repr(accept))
        ...
        if self.format_unit == 'n':
            return """
                ...
                {{{{
                    ...
                    PyObject *iobj = PyNumber_Index({argname});
                    if (iobj != NULL) {{{{
                        ival = PyLong_AsUnsignedLong(iobj);
                        Py_DECREF(iobj);
                    }}}}
                    if (ival == -1 && PyErr_Occurred()) {{{{
                        goto exit;
